...
public class Simulate6 {
    ...
    private Simulate6(Shop shop, PQ<Event> queue) {
        this.shop = shop;
        this.queue = queue;
    }

    public Simulate6(int numServers, List<Pair<Double, Supplier<Double>>> inputTimes, int qmax) {
        ...
        for (int i = 1; i < inputTimes.size() + 1; i++) {
            ...
            Event event = new Arrive(new Customer(i, inputTimes.get(i - 1).first(),
                        serviceTime), inputTimes.get(i - 1).first());
    ...
    public String run() {
        ...
        Simulate6 newSimulate = new Simulate6(updatedShop, newPQ);
